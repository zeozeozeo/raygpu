...
Font LoadFont(const char *fileName)
{
...
#endif
    {
        ...
        if (image.data != NULL) font = LoadFontFromImage(image, (Color){255,255,255,255}, 32);
        UnloadImage(image);
    }
    ...
    //if (isGpuReady)
    //{
    //    if (font.texture.tex == 0) TRACELOG(LOG_WARNING, "FONT: [%s] Failed to load font texture -> Using default font", fileName);
    //    else
    //    {
...
#if defined(SUPPORT_FILEFORMAT_TTF)
    if (strcmp(fileExtLower, ".ttf") == 0 ||
        strcmp(fileExtLower, ".otf") == 0)
    {
